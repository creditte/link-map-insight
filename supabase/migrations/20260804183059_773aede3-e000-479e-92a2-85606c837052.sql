ALTER TABLE public.tenants
  ADD COLUMN IF NOT EXISTS payment_method_captured boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS payment_setup_completed_at timestamptz;

-- Grandfather every existing firm: they signed up before mandatory card capture.
UPDATE public.tenants
SET payment_method_captured = true,
    payment_setup_completed_at = COALESCE(payment_setup_completed_at, now())
WHERE payment_method_captured = false;
